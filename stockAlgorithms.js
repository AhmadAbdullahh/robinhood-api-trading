...
// Your Alpaca API credentials - replace with your own or import from a secure source
const API_KEY = "YOUR_API_KEY";
const API_SECRET = "YOUR_API_SECRET";

// Create an MCP server
const server = new McpServer({
  name: "StockAlgorithms",
  version: "1.0.0"
});
...
server.tool("Run-trading-algorithm",
  { 
    ...
    api_key: z.string().default(API_KEY),
    api_secret: z.string().default(API_SECRET)
  }, 
  async({stock, investment_amount, strategy, api_key, api_secret}) => {
    try {
...
// Connect the server to the transport and start it
